...
export const createAdminMedicine = async (req, res, next) => {
  try {
    ...
    if (!name) { res.status(400); throw new Error('Medicine name is required'); }
...
export const getAllPrices = async (req, res, next) => {
  try {
    const prices = await Price.find().populate('medicine', 'name dosage category').populate('pharmacy', 'name address').sort({ createdAt: -1 });
    res.json(prices);
  } catch (error) { next(error); }
};
...
export const upsertPrice = async (req, res, next) => {
  try {
    ...
    if (existing) {
      ...
      result = await Price.create({ medicine: medicineId, pharmacy: pharmacyId, price, discount: discount || 0 });
    }
    res.json(result);
  } catch (error) { next(error); }
};
...
export const getAlerts = async (req, res, next) => {
  try {
    const alerts = [];
    const outOfStock = await Medicine.find({ stockStatus: 'Out of Stock' }).select('name category dosage');
    outOfStock.forEach(m => alerts.push({
      id: m._id, type: 'low_stock', severity: 'critical',
      message: `${m.name} (${m.dosage}) is OUT OF STOCK`,
      category: m.category, timestamp: m.updatedAt
    }));
