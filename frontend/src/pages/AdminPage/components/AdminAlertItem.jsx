...
const AdminAlertItem = ({ alert, isDarkMode, onResolve }) => {
  ...
  return (
    <div className={`flex flex-col md:flex-row items-start md:items-center gap-8 p-10 rounded-[3rem] transition-all duration-500 group relative overflow-hidden ${
      isDarkMode 
      ...
      <div className={`w-20 h-20 rounded-[30px] flex items-center justify-center shrink-0 transition-all duration-500 ${
        isDarkMode 
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-4 mb-3">
          <span className={`px-4 py-1.5 rounded-full text-[0.65rem] font-black uppercase tracking-[0.2em] flex items-center gap-2.5 ${
        ...
        <h4 className={`text-[1.3rem] font-black leading-tight mb-4 tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>
          {alert.message}
        </h4>
      ...
      <div className="flex items-center gap-6 self-end md:self-center">
        ...
        <ChevronRight size={24} strokeWidth={3} className="text-slate-300 group-hover:translate-x-2 transition-transform opacity-40" />
      </div>
    </div>
  );
};
