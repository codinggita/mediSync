...
const SERVICES = [
  { label: 'API Gateway',     status: 'Operational', uptime: '99.98%', icon: Wifi,          color: 'text-[#2A7FFF] bg-green-50' },
  { label: 'Database',        status: 'Operational', uptime: '99.95%', icon: Server,         color: 'text-[#2A7FFF] bg-green-50' },
  { label: 'Auth Service',    status: 'Operational', uptime: '100%',   icon: Clock,          color: 'text-[#2A7FFF] bg-green-50' },
  { label: 'Storage (CDN)',   status: 'Degraded',    uptime: '97.1%',  icon: AlertTriangle,  color: 'text-[#F59E0B] bg-amber-50' },
];
    ...
    <div className="bg-white rounded-[14px] border border-gray-100 shadow-sm p-5 admin-card flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {SERVICES.map(({ label, status, uptime, icon: Icon, color }) => (
          <div key={label} className="flex items-center justify-between p-2.5 rounded-[10px] bg-[#F8FAFC] border border-gray-100">
            ...
            <span className={`text-[11px] font-bold ${status === 'Operational' ? 'text-[#2A7FFF]' : 'text-[#F59E0B]'}`}>
              {uptime}
            </span>
          </div>
        ))}
      ...
      {/* Error logs */}
      <div>
        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Recent Logs</p>
        <div className="flex flex-col gap-1.5">
          {LOGS.map((log, i) => (
            <div key={i} className={`flex items-start gap-2 p-2 rounded-[8px] border ${LOG_STYLE[log.level]}`}>
