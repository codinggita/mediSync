  ...
  return (
    <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[3rem] p-10 shadow-[20px_20px_40px_#cbced1,-20px_-20px_40px_#ffffff] dark:shadow-[20px_20px_40px_#0a0f1d,-20px_-20px_40px_#202d47] border border-white/40 dark:border-white/5 relative overflow-hidden">
      ...
      <div className="flex flex-col gap-5 relative z-10">
        {criticalCases.map(c => (
          <div key={c.id} className="bg-[#ecf0f3] dark:bg-[#151E32] p-6 rounded-[24px] shadow-[8px_8px_16px_#cbced1,-8px_-8px_16px_#ffffff] dark:shadow-[8px_8px_16px_#0a0f1d,-8px_-8px_16px_#202d47] border-l-4 border-l-red-500 relative overflow-hidden group hover:-translate-y-1 transition-transform">
            ...
            <h4 className="text-[1.1rem] font-black text-slate-900 dark:text-white leading-tight">{c.name}</h4>
            <p className="text-[0.85rem] font-bold text-red-500 mt-1 uppercase tracking-wider">{c.condition}</p>
            <div className="mt-5 pt-4 border-t-2 border-slate-200/50 dark:border-slate-800/50 flex justify-between items-center relative z-10">
              <span className="text-[0.65rem] font-black text-slate-500 uppercase tracking-widest bg-white/50 dark:bg-black/20 px-3 py-1 rounded-md">{c.status}</span>
              <button 
