    ...
    <main className="flex-1 overflow-y-auto bg-[#ecf0f3] dark:bg-[#0B1121] transition-colors duration-300 p-6 lg:p-8 space-y-10 scrollbar-hide pb-24 md:pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
           ...
           <p className="text-[0.85rem] text-[#2A7FFF] font-bold uppercase tracking-[0.2em] mt-1">Clinical Overview Dashboard</p>
        </div>
          ...
          <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[3rem] p-10 shadow-[20px_20px_40px_#cbced1,-20px_-20px_40px_#ffffff] dark:shadow-[20px_20px_40px_#0a0f1d,-20px_-20px_40px_#202d47] border border-white/40 dark:border-white/5 overflow-hidden relative">
             ...
             <div className="flex justify-between items-center mb-10 relative z-10">
                <div>
                   ...
                   <p className="text-[0.75rem] text-slate-400 font-bold uppercase tracking-widest mt-2 ml-14">Records uploaded past 7 days</p>
                </div>
             </div>
             <div className="h-64 w-full flex items-end justify-between gap-4 px-4 relative z-10 border-b-2 border-slate-200/50 dark:border-slate-800/50 pb-4">
                {[40, 65, 30, 85, 55, 90, 75].map((height, i) => (
                   <div key={i} className="w-full flex flex-col items-center gap-3 group cursor-pointer h-full justify-end">
                      <div className="w-full bg-[#ecf0f3] dark:bg-[#151E32] shadow-[inset_4px_4px_8px_#cbced1,inset_-4px_-4px_8px_#ffffff] dark:shadow-[inset_4px_4px_8px_#0a0f1d,inset_-4px_-4px_8px_#202d47] rounded-xl relative overflow-hidden transition-all group-hover:scale-105" style={{ height: `${height}%` }}>
                         <div className="absolute bottom-0 w-full bg-gradient-to-t from-[#2A7FFF] to-[#4DB8FF] opacity-90 transition-all duration-500 group-hover:from-[#1A6FFF] group-hover:to-[#2A7FFF] rounded-xl" style={{ height: '100%' }} />
                      </div>
                      <span className="text-[0.65rem] font-black text-slate-400 group-hover:text-[#2A7FFF] uppercase tracking-wider transition-colors">Day {i+1}</span>
                   </div>
                ))}
             </div>
          </div>
          ...
          <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[3rem] p-10 shadow-[20px_20px_40px_#cbced1,-20px_-20px_40px_#ffffff] dark:shadow-[20px_20px_40px_#0a0f1d,-20px_-20px_40px_#202d47] border border-white/40 dark:border-white/5 relative overflow-hidden">
             ...
             <div className="flex flex-col gap-5 relative z-10">
                {recentActivity.map(activity => (
                   <div key={activity.id} className="flex items-center justify-between p-5 rounded-[24px] bg-[#ecf0f3] dark:bg-[#151E32] shadow-[8px_8px_16px_#cbced1,-8px_-8px_16px_#ffffff] dark:shadow-[8px_8px_16px_#0a0f1d,-8px_-8px_16px_#202d47] border border-white/40 dark:border-white/5 hover:translate-x-2 transition-transform cursor-pointer group">
                      <div className="flex items-center gap-5">
                         <div className={`w-12 h-12 rounded-[18px] flex items-center justify-center shrink-0 shadow-inner ${activity.isCritical ? 'bg-red-500 text-white shadow-red-500/50' : 'bg-[#2A7FFF]/10 dark:bg-white/5 text-[#2A7FFF] dark:text-[#4DB8FF]'}`}>
                         ...
                         <div>
                            ...
                            <p className="font-bold text-[0.8rem] text-slate-500 dark:text-slate-400 mt-1">{activity.detail}</p>
                         </div>
                      </div>
                      <div className="flex items-center gap-4">
