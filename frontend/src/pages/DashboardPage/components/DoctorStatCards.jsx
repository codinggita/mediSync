    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {statCards.map((stat, index) => (
        ...
        >
          ...
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${stat.bg} mb-6 shadow-sm border border-white/20 group-hover:scale-110 transition-transform duration-500 relative z-10`}>
            <stat.icon size={26} color={stat.color} />
          </div>
          
          <div className="relative z-10">
            <p className="text-[0.7rem] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">{stat.label}</p>
