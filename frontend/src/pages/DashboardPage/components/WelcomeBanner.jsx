...
const WelcomeBanner = ({ user, stats, greeting, firstAidImg }) => {
  return (
    <div className="relative bg-gradient-to-br from-[#2A7FFF] via-[#1C71E1] to-[#1565C0] rounded-[40px] p-10 mb-2 overflow-hidden shadow-[20px_20px_40px_rgba(42,127,255,0.15)] group">
      ...
      <div className="absolute -right-20 -bottom-20 w-[400px] h-[400px] opacity-30 pointer-events-none animate-pulse">
         <img src={healthAbstractImg} alt="Abstract" className="w-full h-full object-contain rotate-12" />
      </div>
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10">
        <div className="max-w-2xl">
          <div className="flex items-center gap-3 mb-5">
             <div className="px-4 py-1.5 bg-white/20 backdrop-blur-xl rounded-full text-[0.65rem] font-black text-white uppercase tracking-[0.3em] border border-white/30 flex items-center gap-2.5 shadow-xl">
          ...
          <p className="text-white/80 text-[1rem] sm:text-[1.1rem] font-medium max-w-lg leading-relaxed">
            Your biometrics are <span className="text-white font-black underline decoration-4 decoration-green-400 underline-offset-8">synchronized and stable</span>. You have {stats.appointments} appointments today.
          </p>
          
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-5 mt-8">
             <button className="px-6 py-3 sm:px-8 sm:py-4 bg-white text-[#2A7FFF] rounded-xl sm:rounded-[22px] font-black text-[0.85rem] sm:text-[0.9rem] shadow-[0_10px_25px_rgba(255,255,255,0.3)] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2">
                View Medical Vault <ArrowRight size={18} />
             </button>
             <button className="px-6 py-3 sm:px-8 sm:py-4 bg-white/10 backdrop-blur-xl border border-white/20 text-white rounded-xl sm:rounded-[22px] font-black text-[0.85rem] sm:text-[0.9rem] hover:bg-white/20 transition-all shadow-inner text-center">
                Daily Vitals
             </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8">
           <div className="hidden xl:block w-48 h-48 relative group/asset">
