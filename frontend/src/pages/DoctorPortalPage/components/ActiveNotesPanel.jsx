  ...
  return (
    <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[3.5rem] p-8 shadow-[20px_20px_40px_#cbced1,-20px_-20px_40px_#ffffff] dark:shadow-[20px_20px_40px_#0a0f1d,-20px_-20px_40px_#202d47] border border-white/40 dark:border-white/5 relative overflow-hidden flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-5 max-h-64 overflow-y-auto pr-2 scrollbar-hide relative z-10">
        {notes.map((n, i) => (
          <div key={i} className="flex items-start gap-4 p-5 bg-[#ecf0f3] dark:bg-[#151E32] rounded-[1.5rem] shadow-[inset_4px_4px_8px_#cbced1,inset_-4px_-4px_8px_#ffffff] dark:shadow-[inset_4px_4px_8px_#0a0f1d,inset_-4px_-4px_8px_#202d47] border border-white/40 dark:border-white/5 transition-all">
            <div className={`w-10 h-10 rounded-2xl ${n.color.replace('bg-', 'bg-opacity-20 bg-')} bg-white/50 flex items-center justify-center text-[0.75rem] font-black flex-shrink-0 shadow-sm border border-white/40`}>
              {n.avatar}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <p className="text-[0.85rem] font-black text-slate-800 dark:text-white">{n.name}</p>
                <span className="text-[0.65rem] font-bold text-slate-400 uppercase tracking-widest">{n.time}</span>
              </div>
              <p className="text-[0.8rem] text-slate-600 dark:text-slate-400 font-medium leading-relaxed">{n.note}</p>
            </div>
          </div>
        ))}
      </div>
