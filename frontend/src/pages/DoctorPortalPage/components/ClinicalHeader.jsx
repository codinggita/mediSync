...
const ClinicalHeader = ({ refreshing, handleRefresh, activeAppointment, patients, selectedPatientId, setSelectedPatientId }) => (
  <div className="flex items-center justify-between">
    ...
    <div className="flex items-center gap-6">
      <div className="hidden xl:flex flex-col items-end border-r border-slate-300 dark:border-slate-800 pr-6 mr-6">
         <span className="text-[0.65rem] font-black text-slate-400 uppercase tracking-widest">Next Patient</span>
         <span className="text-[0.9rem] font-black text-[#2A7FFF] truncate max-w-[150px]">{activeAppointment?.patient?.name || patients[0]?.name || 'No Pending'}</span>
      </div>
      ...
      <div className="hidden lg:flex items-center gap-4 p-2 bg-[#ecf0f3] dark:bg-[#151E32] rounded-[2rem] shadow-inner border border-white/40">
         <div className="pl-6 pr-4 py-2 border-r border-slate-300 dark:border-slate-700">
            <span className="text-[0.75rem] font-black text-slate-400 uppercase tracking-[0.1em]">Patient Node</span>
         </div>
         <div className="relative group min-w-[200px]">
            ...
            >
               {patients.map(p => (
                 <option key={p._id} value={p._id}>{p.name} ({p.patientId || 'NEW'})</option>
               ))}
            </select>
