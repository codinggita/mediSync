  ...
  <div className="xl:col-span-4 flex flex-col gap-10">
    ...
    <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[2.5rem] p-6 shadow-[10px_10px_20px_#cbced1,-10px_-10px_20px_#ffffff] dark:shadow-[10px_10px_20px_#0a0f1d,-10px_-10px_20px_#202d47] border border-white/40 dark:border-white/5 flex items-start gap-5 relative overflow-hidden group">
      ...
      <div className="relative z-10">
        <h4 className="text-[0.95rem] font-black text-red-500 uppercase tracking-widest mb-1.5 drop-shadow-sm">Critical Allergy</h4>
        <p className="text-[0.8rem] font-bold text-slate-500 dark:text-slate-400 leading-tight">Patient has a severe registered allergy to <span className="text-red-400 font-black">Penicillin</span>.</p>
      </div>
    </div>
    ...
    <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[3.5rem] p-8 shadow-[20px_20px_40px_#cbced1,-20px_-20px_40px_#ffffff] dark:shadow-[20px_20px_40px_#0a0f1d,-20px_-20px_40px_#202d47] border border-white/40 dark:border-white/5 relative overflow-hidden">
      ...
      <div className="flex flex-col gap-5 max-h-[300px] overflow-y-auto scrollbar-hide relative z-10 pr-2">
        {appointments.length === 0 ? (
          <p className="text-[0.8rem] text-slate-400 font-bold uppercase tracking-widest text-center py-4">No pending appointments.</p>
        ) : (
          appointments.map(app => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[0.9rem] font-black text-slate-800 dark:text-white group-hover:text-[#2A7FFF] transition-colors">{app.patient?.name || 'Unknown'}</span>
                <span className={`px-3 py-1 rounded-lg text-[0.6rem] font-black uppercase tracking-widest shadow-inner ${
                  app.status === 'Pending' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'
                }`}>
                  {app.status}
                </span>
              </div>
              <div className="flex items-center gap-4 text-[0.7rem] font-bold text-slate-500 uppercase tracking-widest mt-1">
                <span className="flex items-center gap-1.5"><Clock size={12} className="text-[#2A7FFF]" /> {app.time}</span>
                <span className="flex items-center gap-1.5"><Video size={12} className="text-[#2ECC71]" /> {app.type}</span>
              </div>
            </div>
          ))
        )}
    ...
    <div className="bg-[#ecf0f3] dark:bg-[#151E32] rounded-[3.5rem] p-8 shadow-[20px_20px_40px_#cbced1,-20px_-20px_40px_#ffffff] dark:shadow-[20px_20px_40px_#0a0f1d,-20px_-20px_40px_#202d47] border border-white/40 dark:border-white/5 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        ...
        <span className="px-4 py-1.5 bg-emerald-500/10 text-emerald-500 text-[0.65rem] font-black rounded-xl border border-emerald-500/20 shadow-inner uppercase tracking-widest flex items-center gap-2">
          <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_#2ECC71]" /> Live
        </span>
      </div>
