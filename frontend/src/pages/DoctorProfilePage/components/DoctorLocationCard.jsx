...
const DoctorLocationCard = ({ location, onNavigate }) => {
  return (
    <section className="p-10 rounded-[3.5rem] bg-[#0B1121] dark:bg-black/40 relative overflow-hidden group/map mt-12" onClick={onNavigate}>
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-[#2A7FFF] flex items-center justify-center text-white">
            <MapPin size={20} />
          </div>
          <h3 className="text-[1rem] font-black text-white uppercase tracking-widest">Clinic Coordinates</h3>
        </div>
        <button className="p-3 rounded-xl bg-white/10 text-white hover:bg-[#2A7FFF] transition-all">
          <ArrowUpRight size={20} />
        </button>
      </div>
      <p className="text-[1.1rem] font-bold text-slate-400 mb-8 relative z-10 max-w-lg">{location}</p>
