        ...
        {patients.map(p => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-2 pl-1">
              <h4 className={`text-[1.1rem] font-black ${selectedPatientId === p.id ? 'text-[#2A7FFF]' : 'text-slate-900 dark:text-white group-hover:text-[#2A7FFF]'}`}>{p.name}</h4>
            ...
            <div className="flex flex-wrap gap-2 pl-1">
              {p.conditions.map((c, i) => (
                <span key={i} className={`px-2.5 py-1 rounded-lg text-[0.65rem] font-black uppercase tracking-wider ${selectedPatientId === p.id ? 'bg-[#2A7FFF]/10 text-[#2A7FFF]' : 'bg-white/50 dark:bg-black/20 text-slate-500'} shadow-sm`}>
                  {c}
                </span>
              ))}
            </div>
