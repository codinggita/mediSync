...
const ResetPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Synchronization failed. The reset token may be invalid or expired.');
    } finally {
      setIsLoading(false);
    }
  };
           ...
           {!isSuccess ? (
             <>
               <div className="w-24 h-24 rounded-[32px] bg-slate-50 dark:bg-white/5 flex items-center justify-center mx-auto mb-10 shadow-inner group">
                  <ShieldCheck size={48} className="text-[#2ECC71] group-hover:scale-110 transition-transform duration-500" />
               </div>
