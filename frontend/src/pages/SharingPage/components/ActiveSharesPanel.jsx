...
const initials = (name) => name.split(' ').filter(w => w.startsWith('Dr.') === false).map(w => w[0]).join('').slice(0, 2);
    ...
    <div className="bg-white dark:bg-[#151E32] border border-gray-100 dark:border-slate-700/50 rounded-2xl p-5 shadow-sm transition-colors">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[0.65rem] text-gray-400 mt-0.5">{shares.length} records currently shared</p>
          </div>
        </div>
      ...
      {shares.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {shares.map(({ id, doctor, role, hospital, record, shared, expiresIn, expiryPct, pinEnabled, viewed, status }) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-[0.85rem] font-extrabold text-[#1F2937] dark:text-white truncate">{doctor}</p>
                      <span className="text-[0.62rem] font-bold px-1.5 py-0.5 rounded-full" style={{ color: st.color, backgroundColor: st.bg }}>
                        {st.label}
                      </span>
                    </div>
                    <p className="text-[0.72rem] text-gray-400 font-medium">{role} · {hospital}</p>
                  </div>
                ...
                <div className="mt-3 flex items-center gap-2 flex-wrap text-[0.7rem] text-gray-400">
                  <span className="flex items-center gap-1"><Lock size={10} /> {record}</span>
                  {pinEnabled && (
                    <span className="flex items-center gap-1 text-[#8B5CF6] font-semibold">· PIN ✓</span>
                  )}
                  {viewed && (
                ...
                <div className="mt-3">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[0.65rem] font-bold" style={{ color: barColor(expiryPct) }}>
                      {status === 'expired' ? (
