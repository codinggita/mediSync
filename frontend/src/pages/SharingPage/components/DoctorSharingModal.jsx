...
const RecordListItem = ({ record, isSelected, onSelect }) => (
  ...
  >
    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isSelected ? 'bg-white/20 text-white' : 'bg-[#ecf0f3] dark:bg-[#151E32] text-[#2A7FFF]'}`}>
      <FileText size={22} />
    </div>
    <div className="flex-1 min-w-0">
      <p className={`text-[1rem] font-black truncate ${isSelected ? 'text-white' : 'text-slate-800 dark:text-white'}`}>{record.title}</p>
      <p className={`text-[0.7rem] font-bold uppercase tracking-widest ${isSelected ? 'text-white/70' : 'text-slate-400'}`}>{record.type} · {new Date(record.createdAt).toLocaleDateString()}</p>
    </div>
...
const DoctorSharingModal = ({ 
  show, onClose, selectedDoctor, records, selectedRecordId, 
  setSelectedRecordId, onConfirm, isSharing, successMessage 
}) => {
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      <div className="bg-[#ecf0f3] dark:bg-[#151E32] w-full max-w-xl rounded-[4rem] p-12 shadow-2xl relative z-10 border border-white/20">
        {successMessage ? (
          ...
          <>
            <button onClick={onClose} className="absolute top-8 right-8 w-12 h-12 rounded-2xl bg-white dark:bg-white/5 flex items-center justify-center text-slate-400 hover:text-red-500 shadow-sm transition-all"><XCircle size={24} /></button>
            <div className="flex flex-col items-center text-center mb-10">
              <div className="w-20 h-20 rounded-[2rem] bg-[#2A7FFF]/10 flex items-center justify-center text-[#2A7FFF] mb-6 shadow-inner"><Lock size={32} /></div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="space-y-4 max-h-[250px] overflow-y-auto pr-2 scrollbar-hide">
                  {records.length > 0 ? (
                    records.map(record => (
                      <RecordListItem key={record._id} record={record} isSelected={selectedRecordId === record._id} onSelect={setSelectedRecordId} />
                    ))
                  ) : (
              ...
              <button 
                onClick={onConfirm} disabled={isSharing || !selectedRecordId}
